...
const GameChart = (props) => {
  ...
  const [chartDataCheck, setChartDataCheck] = useState(true)
  const [dataNew, setDataNew] = useState([])
  const [yCrash, setYCrash] = useState()
  ...
  const canvasLoaderPlanRef = useRef(null);
  ...
  useEffect(() => {
    const dataAdd = []
    if (time <= 0 && refreshGame === false && newStartGame === false) {
  ...
  useEffect(() => {
    if (canvasLoaderPlanRef?.current) {
  ...
  return (
    <>
      <div className="chartShow" style={{ paddingLeft: `${time >= -8 && time < 0 ? "0" : "47px"}` }}>
        ...
        {/* <img src={GameBg} class="rotateimage " />    */}
        <img src={GameBg} className={`rotateimage ${newStartGame !== false || crashRocket !== false ? "rotatebg" : ""}`} />
        {
        ...
        <div className='showCanvasShow'>
          {
            time >= -8 && time < 0 ?
              <canvas ref={canvasLoaderPlanRef} height={400} style={{ border: "none", width: "40%", height: "100%" }} />
              :
              <>
                <canvas ref={canvasRef} height={400} style={{ borderWidth: newStartGame === true ? "1px" : "0" }} />
                {
                  newStartGame === true && (
                    <>
